  ...
  while (fail && iter < M)
  {
    ...
    for (int i = 0; i < t && !fail; i++)
    {
      ...
      { // Step a
        ...
        z[3] = rr.w();
        // Step b
        if (check_norm(z[0], UBound1))
        {
          cout << "F0:" << i << " " << j << endl;
          fail = true;
        }
        if (check_norm(z[1], UBound2))
        {
          cout << "F1:" << i << " " << j << endl;
          fail = true;
        }
        if (check_norm(z[2], UBound3))
        {
          cout << "F2:" << i << " " << j << endl;
          fail = true;
        }
        if (check_norm(z[3], UBound3))
        {
          cout << "F3:" << i << " " << j << endl;
          fail = true;
        }
        ...
        for (int k = 0; k < (*P).num_players(); k++)
        {
          ...
          if (k != my_num)
          {
            ...
            else
            {
              ...
              if (z[0].infinity_norm() > UBound1)
              {
                cout << "R0" << endl;
                throw bad_enccommit();
              }
              if (z[1].infinity_norm() > UBound2)
              {
                cout << "R1" << endl;
                throw bad_enccommit();
              }
              if (z[2].infinity_norm() > UBound3)
              {
                cout << "R2" << endl;
                throw bad_enccommit();
              }
              if (z[3].infinity_norm() > UBound3)
              {
                cout << "R3" << endl;
                throw bad_enccommit();
              }
