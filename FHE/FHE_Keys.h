...
public:
  ...
  FHE_SK operator+(const FHE_SK &x) const
  {
    FHE_SK res = *this;
    res += x;
    return res;
  ...
  friend ostream &operator<<(ostream &o, const FHE_SK &)
  {
...
public:
  ...
  FHE_PK(const FHE_Params &pms, int p) : FHE_PK(pms, bigint(p))
  {
  }

  template <class FD>
  FHE_PK(const FHE_Params &pms, const FD &FTD) : FHE_PK(pms, FTD.get_prime())
  {
  }
...
public:
  ...
  FHE_KeyPair(const FHE_Params &params, const bigint &pr) : pk(params, pr), sk(params, pr)
  {
  }
...
/// Create and generate a new key pair from FHE params
unique_ptr<FHE_KeyPair> new_keypair(const FHE_Params &params);
/// Get the public key from a key pair
unique_ptr<FHE_PK> get_pk(const FHE_KeyPair &pair);
