...
{
  auto res = unique_ptr<FHE_KeyPair>(new FHE_KeyPair(params));
  res->generate();
...
{
  return unique_ptr<FHE_PK>(new FHE_PK(pair.pk));
}
...
FHE_SK::FHE_SK(const FHE_Params &pms) : FHE_SK(pms, pms.get_plaintext_modulus())
{
}
...
FHE_PK::FHE_PK(const FHE_Params &pms) : FHE_PK(pms, pms.get_plaintext_modulus())
{
}
...
{
  if ((*params) != *(x.params) or pr != x.pr or a0 != x.a0 or b0 != x.b0 or Sw_a != x.Sw_a or Sw_b != x.Sw_b)
  {
